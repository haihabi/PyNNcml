...
class WetDryNetwork(BaseCMLProcessingMethod):
    """
    The module generates a wet dry network that was presented in [1]
    ...
    :param normalization_cfg: a class pnc.neural_networks.InputNormalizationConfig which
                               holds the normalization parameters.
    ...
    :param tn_affine: boolean that state if time normalization has affine transformation.
    ...
    def __init__(self, n_layers: int, rnn_type: pnc.neural_networks.DNNType,
        ...
        super(WetDryNetwork, self).__init__(input_data_type,input_rate,output_rate)
