...
class TestPowerLaw(unittest.TestCase):
    ...
    def test_h_v_diff(self):
        ...
        res_h = pl(torch.as_tensor(att * np.ones(TestPowerLaw.n_samples)).float(), length, freq, 'H').numpy()
        res_v = pl(torch.as_tensor(att * np.ones(TestPowerLaw.n_samples)).float(), length, freq, 'V').numpy()
