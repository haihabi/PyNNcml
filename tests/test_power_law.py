...
class TestPowerLaw(unittest.TestCase):
    ...
    def test_rmin(self):
        att = 5
        length = 8.3
        pl = tr.power_law.PowerLaw(tr.power_law.PowerLawType.INSTANCE, 1000)
    ...
    def test_powerlaw(self):
        att = 5
        length = 8.3
        pl = tr.power_law.PowerLaw(tr.power_law.PowerLawType.INSTANCE, TestPowerLaw.r_min)
        res = pl(torch.as_tensor(att * np.ones(TestPowerLaw.n_samples)).float(), length, TestPowerLaw.freq, 0).numpy()
    ...
    def test_powerlaw_min_max(self):
        att = 5
        length = 8.3
        pl = tr.power_law.PowerLaw(tr.power_law.PowerLawType.MAX, TestPowerLaw.r_min)
        res = pl(torch.as_tensor(att * np.ones(TestPowerLaw.n_samples)).float(), length, TestPowerLaw.freq, 0).numpy()
